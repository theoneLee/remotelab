...
import {
  loadSessions, saveSessions, generateId, sessionExists, killSession,
  getSessionSocketName, getSessionsByFolder, spawnSessionTtyd,
  killSessionTtyd,
} from './sessions.mjs';
...
import { loginPage, dashboardPage, folderViewPage, chatViewPage } from './templates.mjs';
...
const clientAssetsDir = join(dirname(fileURLToPath(import.meta.url)), 'client');
const CLIENT_ASSETS = new Map([
  ['chat-view.mjs', { file: 'chat-view.mjs', type: 'text/javascript; charset=utf-8' }],
  ['chat-approvals.mjs', { file: 'chat-approvals.mjs', type: 'text/javascript; charset=utf-8' }],
  ['chat-utils.mjs', { file: 'chat-utils.mjs', type: 'text/javascript; charset=utf-8' }],
]);
...
function serveClientAsset(pathname, res) {
  const assetName = pathname.slice('/assets/'.length);
  const asset = CLIENT_ASSETS.get(assetName);
  ...
  res.end(readFileSync(join(clientAssetsDir, asset.file), 'utf8'));
  return true;
}
...
export async function handleRequest(req, res) {
  ...
  if (pathname.startsWith('/assets/')) {
    if (serveClientAsset(pathname, res)) return;
  }
  ...
  if (pathname === '/api/sessions' && req.method === 'POST') {
    ...
    try {
      ...
      const resolvedFolder = resolveFolderPath(folder);
  ...
  if (pathname.startsWith('/chat/')) {
    const encodedPath = pathname.slice('/chat/'.length);
    if (!encodedPath) {
      res.writeHead(400, { 'Content-Type': 'text/plain' });
      res.end('Folder path required');
      return;
