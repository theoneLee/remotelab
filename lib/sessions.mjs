...
import { TOOL_PATH } from './tools.mjs';
...
function findTtydPath() {
  ...
  // Prefer a user-local ttyd if present (e.g. installed via setup-linux.sh
  // into ~/.local/bin) before falling back to whatever is on PATH.
  const preferred = [
    join(home, '.local', 'bin', 'ttyd'),
  ];
  ...
  try {
    const env = TOOL_PATH
      ? { ...process.env, PATH: TOOL_PATH }
      : process.env;
    return execFileSync('which', ['ttyd'], {
      encoding: 'utf8',
      timeout: 3000,
      env,
    }).trim();
    ...
    for (const p of ['/opt/homebrew/bin/ttyd', '/usr/local/bin/ttyd', '/snap/bin/ttyd']) {
...
export async function spawnSessionTtyd(session) {
  ...
  const args = [
    '--writable',
    '--port', String(port),
    '--interface', getLoopbackInterface(),
    '--ping-interval', '30',
    '--max-clients', '0',
  ...
  const ttyd = spawn(ttydPath, args, {
    ...
    env: {
      ...process.env,
      PATH: TOOL_PATH || process.env.PATH,
    },
  });
...
export function loadSessions() {
  try {
    ...
    return sanitizeSessions(JSON.parse(readFileSync(SESSIONS_FILE, 'utf8')));
  } catch (err) {
...
export function saveSessions(sessionsList) {
  try {
    ...
    writeFileSync(SESSIONS_FILE, JSON.stringify(sanitizeSessions(sessionsList), null, 2), 'utf8');
  } catch (err) {
